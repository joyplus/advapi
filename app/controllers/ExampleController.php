...
class ExampleController extends RESTController{
	...
	public function get(){
        ...
        $phql = "SELECT * FROM ProductTypes";
        $modelsManager = $this->di->get('modelsManager');
        $productTypes = $modelsManager->executeQuery($phql);
    ...
    public function insert(){
        ...
        $phql = "INSERT INTO ProductTypes (id, name) VALUES (:id:, :name:)";
        $modelsManager = $this->di->get('modelsManager');

        $status = $modelsManager->executeQuery($phql, array(
            'id' => '9',
            'name' => 's'
        ));
        ...
        if ($status->success() == true) {
            ...
            return $this->respond(array('status' => 'ERROR', 'messages' => $errors));
