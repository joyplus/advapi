...
class MDClickController extends RESTController {
	...
	private function track(){
		...
		switch($req->get('type')){
			...
			case 'normal':
				$this->reporting_db_update($display_ad, $request_settings, $zone_detail->publication_id, $req->get('zone_id'), $req->get('campaign_id'), $req->get('ad_id'), '', 0, 0, 0, 1);
				break;
		
			case 'network':
				$this->reporting_db_update($display_ad, $request_settings, $zone_detail->publication_id, $req->get('zone_id'), $req->get('campaign_id'), '', $req->get('network_id'), 0, 0, 0, 1);
				break;
		
			case 'backfill':
				$this->reporting_db_update($display_ad, $request_settings, $zone_detail->publication_id, $req->get('zone_id'), '', '', $req->get('network_id'), 0, 0, 0, 1);
				break;
