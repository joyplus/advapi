...
class MDCreativeController extends RESTController {
	...
	private function process($zone) {
		...
		if($campaign) {
			$this->logDebug("[process] find campaigns id->" . $campaign->campaiagn_id);
	...
	private function sendBeanstalk($zone, $ad) {
		...
		$log['user_pattern'] = '';
		$log['date'] = time();
		$log['operation_type'] = self::OPERATION_TYPE_CREATIVE_URL;
		$log['operation_extra'] = '';
	...
	private function findCampaigns($zone, $date) {
		...
		$result = $this->modelsManager->createBuilder()->from('Campaigns')
	...
	private function logDebug($log) {
		$this->log("[MDCreativeController]" . $log, Phalcon\Logger::DEBUG);
	}
}